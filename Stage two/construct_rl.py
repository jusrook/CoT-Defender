...
def get_model_inputs(prompts,n=1):
    ...
    for prompt in prompts:
        ...
        for _ in range(n):
            texts.append(text)
    model_inputs = tokenizer(texts, padding=True, return_tensors="pt").to("cuda")
    return model_inputs
...
def psoe(output,tags,generations,input_qs):
    ...
    def get_logits(logits):
        ...
        for j in range(logits[0].shape[0]):
            for i in range(len(logits)):
                if i + 1 < len(logits) and "Decision" == tokenizer.decode(torch.argmax(logits[i][j])) and ":" == tokenizer.decode(torch.argmax(logits[i+1][j])):
                    pos_ids.append(i)
                    break
                if i == len(logits)-1:
                    pos_ids.append(0)
        try:
            ...
            response_logits = [logits[d+8][i] for i,d in enumerate(pos_ids)]
            decision_logits = [logits[d][i] for i,d in enumerate(pos_ids)]
            output_logits = [logits[d+10][i] for i,d in enumerate(pos_ids)]
        except IndexError:
    ...
    def get_score(logit,tag,response_logit,thought_logit,decision_logit,output_logit,generation,input_q):
        ...
        answer = tokenizer(" answer")["input_ids"][-1]
        refuse = tokenizer(" refuse")["input_ids"][-1]
        res = tokenizer("Response")["input_ids"][-1]
        th = tokenizer("Thought")["input_ids"][-1]
        dec = tokenizer("Decision")["input_ids"][-1]
        output = [358, 1634, 32286]
...
def construct_harmful_rl(ds_given, tag, num = 1, reapeat_num = 6, attack_prompt_template = attack_prompt_template):
    ...
    while (i!=len(ds_given) or len(left_prompts)!=0):
        ...
        for n,score in enumerate(scores_array):
            ...
            if range_value>2.7 and max(score)>7.5:
...
def construct_harmless_rl(ds_given, tag, num = 1, reapeat_num = 6):
    ...
    while (i!=len(ds_given) or len(left_prompts)!=0):
        ...
        for n,score in enumerate(scores_array):
            range_value = np.ptp(score)
            if max(score)>7.5 and range_value>1.5:
                prompt.append(current_prompts[n])
                chosen.append(responses[n][np.argmax(score)].item())
                rejected.append(responses[n][np.argmin(score)].item())
                removed.append(n)
